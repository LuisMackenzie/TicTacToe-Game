...
public class OnePlayerActivity extends AppCompatActivity {
    ...
    private void jugadaListener() {
        listenerJugada = db.collection("jugadas")
                .document(jugadaId)
                .addSnapshotListener(OnePlayerActivity.this, new EventListener<DocumentSnapshot>() {
                    @Override
                    public void onEvent(@Nullable DocumentSnapshot snapshot, @Nullable FirebaseFirestoreException e) {
                        if(e != null) {
                            Toast.makeText(OnePlayerActivity.this, "Error al obtener los datos de la jugadas", Toast.LENGTH_SHORT).show();
                            return;
                        }
    ...
    private void getPlayerNames() {
        ...
        db.collection("users")
                .document(uid)
                .get()
                .addOnSuccessListener(OnePlayerActivity.this, new OnSuccessListener<DocumentSnapshot>() {
                    @Override
    ...
    private void updatePlayersUI() {
        ...
        if(jugada.isTurnoP1()) {
            ...
            binding2.textViewPlayer1.setText(playerOneName);
            binding2.textViewPlayer2.setText(playerTwoName);
            binding2.ivLottieImage.playAnimation();
        }
    ...
    public void casillaSeleccionada(View view) {
        ...
        db.collection("jugadas")
                .document(jugadaId)
                .set(jugada)
                .addOnSuccessListener(OnePlayerActivity.this, new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
                        // Aqui faltan cosas

                    }
                }).addOnFailureListener(OnePlayerActivity.this, new OnFailureListener() {
            @Override
    ...
    private void actualizarJugadaMaquina(int numeroCasilla) {

        // int posicionCasilla = Integer.parseInt(numeroCasilla);

        if (jugada.getCeldas().get(numeroCasilla) != 0) {
            Toast.makeText(this, "Seleccione una casilla libre", Toast.LENGTH_SHORT).show();
        } else {
            if (jugada.isTurnoP1()) {
                animCasillas.get(numeroCasilla).setRepeatCount(0);
                animCasillas.get(numeroCasilla).setAnimation("green_ckeck.json");
                // animCasillas.get(posicionCasilla).setVisibility(View.VISIBLE);
                animCasillas.get(numeroCasilla).playAnimation();
                // casillas.get(posicionCasilla).setImageResource(R.drawable.ic_player_one);
                jugada.getCeldas().set(numeroCasilla, 1);
            } else {
                animCasillas.get(numeroCasilla).setRepeatCount(0);
                animCasillas.get(numeroCasilla).setAnimation("red_cross.json");
                // animCasillas.get(posicionCasilla).setVisibility(View.VISIBLE);
                animCasillas.get(numeroCasilla).playAnimation();
                // casillas.get(posicionCasilla).setImageResource(R.drawable.ic_player_two);
                jugada.getCeldas().set(numeroCasilla, 2);
            }
    ...
    private void iaJugada() {
        ...
        // se ha de notificar a firebase HECHO
        // Actualizar en Firestore los datos de la jugada
        db.collection("jugadas")
                .document(jugadaId)
                .set(jugada)
